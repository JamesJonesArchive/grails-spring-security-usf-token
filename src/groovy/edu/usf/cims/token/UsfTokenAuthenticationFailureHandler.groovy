 ...
 * @author james
 */
class UsfTokenAuthenticationFailureHandler implements AuthenticationFailureHandler {
    ...
    public UsfTokenAuthenticationFailureHandler() {
        
    }
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,AuthenticationException exception) throws IOException, ServletException {
        println "Running the UsfTokenAuthenticationFailureHandler"
        // authenticationEntryPoint.commence(request,response,exception)
        String contentType = request.getContentType();
        logger.info(contentType);
        // response.sendError( HttpServletResponse.SC_UNAUTHORIZED, "Unauthorized" );
        if(isPreflight(request)){
            print "Running PreFlight Response"
            def headers = [:]
            ...
            println "Running Regular Ajax Unauthorized 401"
